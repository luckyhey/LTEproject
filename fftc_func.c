						...
						unsigned int * fftc_output_data_address)
{
	...
	fft_config(dft_list,/*输入的DFT/IDFTsize，仅支持DFT_sz_list里面的各种size*/
	           ...
	           vxx_shift_PARA.shift_data_len,//hey 原先为1204,/*输入数据长度必须是4的倍数*/
	           IDFT_MODE,/*DFT/IDFT*/
	           fftc_output_data_address,/*输出数据地址,使用时注意输出数据要偏移4个int才是真正的结果*/
	           CSL_FFTC_A,
	           vxx_shift_PARA.ZERO_PAD_VALUE,/*zero-pad value*/
	           vxx_shift_PARA.VARIABLE_SHIFT_INPUT,/*VARIABLE_SHIFT_INPUT = 1448/2*/
	           cp_len,
	           0,
	           1);
    ...
    if(N_SYM_PER_SLOT >= total_symbol)//总符号数<=7时
    {
    	...
    	ok_flag = fft_config(dft_list,/*输入的DFT/IDFTsize，仅支持DFT_sz_list里面的各种size*/
    	                         	...
    	                         	vxx_shift_PARA.shift_data_len,/*输入数据长度必须是4的倍数*/
    	                         	 IDFT_MODE,/*DFT/IDFT*/
    								 fftc_output_data_address,/*输出数据地址,使用时注意输出数据要偏移4个int才是真正的结果*/
    								 CSL_FFTC_A,
    								 vxx_shift_PARA.ZERO_PAD_VALUE,/*zero-pad value*/
